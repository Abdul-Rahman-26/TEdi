    ...
    if len(sequence[0]) > 0:
        ...
        for key, value in tokenizer.word_index.items():
            if value == preds:
                predicted_word = key
                break
        if predicted_word in tokenizer.word_index and predicted_word not in text.split():
...
def update_suggestion_boxes(event=None):
    ...
    if user_input:
        ...
        while len(set(predicted_words)) < 3:
            predicted_word = predict_next_word(model, tokenizer, " ".join([tokenizer.index_word.get(idx, "") for idx in last_words]))
            if predicted_word:
                predicted_word_index = tokenizer.word_index.get(predicted_word, 0)
                last_words.append(predicted_word_index)
                predicted_words.append(predicted_word)
            else:
                break
        # randomly shuffle the predicted words
        random.shuffle(predicted_words)
        # assign the shuffled words to the suggestion boxes
...
def insert_suggestion_word(event=None):
    ...
    if suggestion_word:
        if user_input:
            ...
            logger.info(f'{last_words} {user_input}')
        else:
        ...
        if user_input:
            ...
            user_input = " ".join([last_words[0], typed_text])
            with open('user_input.txt', 'a') as file:
                file.write(user_input + " ")
            logger.info(user_input)
        txtarea.insert(tk.END, typed_text)
        typed_text = ""
